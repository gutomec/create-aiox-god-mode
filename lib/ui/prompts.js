 ...
 * @returns {Promise<{ name: string, squads: string[], aiTool: string, installOmc: boolean }>}
 */
export async function promptProjectConfig(projectName) {
  const questions = [
    ...
    },
    {
      type: 'confirm',
      name: 'installOmc',
      message: 'Install oh-my-claudecode (multi-agent orchestration)?',
      initial: true,
    },
  ];
 ...
 * @returns {{ name: string, squads: string[], aiTool: string, installOmc: boolean }}
 */
